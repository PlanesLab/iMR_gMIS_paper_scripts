function [G, G_ind, G_rxns, related, n_genes_KO, G_time, iMRmodel_info] = buildGmatrix_iMRmodel_gMIS(model_name, model_struct, regulatoryNetwork, nLayers, pos, numWorkers)
% Build the G matrix in eGPR networks required for the calculation of
% genetic Minimal Intervention Sets (gMISs) on integrated metabolic and regulatory
% models.
%
% USAGE:
%
%    [G, G_ind, G_rxns, related, n_genes_KO, G_time, iMRmodel_info] = buildGmatrix_iMRmodel_gMIS(model_name, model_struct, regulatoryNetwork, nLayers, pos, numWorkers)
%
% INPUTS:
...
%    pos:                   GPR position in which none regulatory information is added.
%    numWorkers:            Number of CPUs employed by Cplex.
%
% OUTPUTS:
%    G:                 G matrix.
...
% EXAMPLE:
%
%    [G, G_ind, G_rxns, related, n_genes_KO, G_time, iMRmodel_info] = buildGmatrix_iMRmodel_gMIS(Human1_TRRUST1, HumanGEM, TRRUST_interactions, 1, 10, 8)
%
% .. Authors:
%       - Naroa Barrena, 29/07/2024, University of Navarra, TECNUN School of Engineering
%       - Luis V. Valcarcel, 29/07/2024, University of Navarra, TECNUN School of Engineering.
%       - Francisco J. Planes, 29/07/2024, University of Navarra, TECNUN School of Engineering.
...
[BooleanRulesParsed, auxKO_Nodes, auxKin_Nodes] = AddingAuxKO_Nodes(BooleanRulesParsed, inputs, outputs);
...
auxKin_Node_off = sort(strcat(regexprep(BooleanRulesParsed2, ' =.*$', ''), '_auxKin_off'));
auxKin_Nodes = [strcat(auxKin_Nodes, '_on'); auxKin_Node_off];

auxKO_Nodes = [auxKO_Nodes; auxKin_Nodes];
...
function [BooleanRules, auxKO_Node, auxKin_Node] = AddingAuxKO_Nodes(BooleanRules, inputs, outputs)
...
auxKin_Node = strcat(regexprep(BooleanRules2, ' =.*$', ''), '_auxKin');

BooleanRules_new = strcat(regexprep(BooleanRules2 ,'= ', '= ('), ') | ', {' '}, auxKin_Node);
BooleanRules_new = strcat(regexprep(BooleanRules_new ,'= ', '= ('), ') & ', {' '}, auxKO_Node);
...
if ~isempty(inputs)
    ...
    BooleanRulesInput = strcat(inputs, ' = ', {' '} , inputs, '_auxKO', {' | '}, inputs, '_auxKin');
    auxKO_NodeInput = strcat(inputs, '_auxKO');
    auxKin_NodeInput = strcat(inputs, '_auxKin');
    BooleanRules = [BooleanRules_out; BooleanRules_new; BooleanRulesInput];
    auxKO_Node = unique(sort([auxKO_Node; auxKO_NodeInput]));
    auxKin_Node = unique(sort([auxKin_Node; auxKin_NodeInput]));
end
...
% for i = 1:length(auxKin_Nodes)
%     model = addReaction(model, strcat('r_', auxKin_Nodes{i,1}), ...
%         'reactionName', strcat('r_', auxKin_Nodes{i,1},'_input'), 'metaboliteList', {strcat(auxKin_Nodes{i,1}, '[c]')},...
%         'lowerBound', 0, 'upperBound', 10000, 'stoichCoeffList', 1, 'printLevel', 0);
%     rxn_set{i + length(auxKO_Nodes),1} = strcat('r_', auxKin_Nodes{i,1});
% %     pos = pos+1;   
% end

for i = 1:length(inputs)
    model = addReaction(model, strcat('r_', inputs{i,1}, '_off_input'), ...
        'reactionName', strcat('r_', inputs{i,1}, '_off_input'), 'metaboliteList', {strcat(inputs{i,1}, '_auxKO_off[c]')},...
        'lowerBound', 0, 'upperBound', 100, 'stoichCoeffList', 1, 'printLevel', 0);  
%     rxn_set{length(auxKO_Nodes) + length(auxKin_Nodes) + i,1} = strcat('r_', inputs{i,1}, '_off_input');
end
...
auxKO_on_pos = find(contains(auxKO_Nodes, '_auxKO_on'));
auxKO_off_pos = setdiff(1:(numel(auxKO_Nodes)/2), auxKO_on_pos);

auxKin_on_pos = find(contains(auxKO_Nodes, '_auxKin_on'));
auxKin_off_pos = setdiff((numel(auxKO_Nodes)/2)+1:numel(auxKO_Nodes), auxKin_on_pos);
...
cons.auxKin = cons.auxKO(end)+1:cons.auxKO(end)+length(auxKin_on_pos);
cons.auxKO_Kin = cons.auxKin(end)+1:cons.auxKin(end)+length(auxKO_on_pos);
% cons.auxKin_KO = cons.auxKO_Kin(end)+1:cons.auxKO_Kin(end)+length(auxKO_on_pos);
cons.forceLength = cons.auxKO_Kin(end)+1:cons.auxKO_Kin(end)+1;
...
A(cons.auxKin, var.zp(auxKin_on_pos)) = speye(length(auxKin_on_pos));
A(cons.auxKin, var.zp(auxKin_off_pos)) = speye(length(auxKin_off_pos));

A(cons.auxKO_Kin, var.zp(auxKO_on_pos)) = speye(length(auxKO_on_pos));
A(cons.auxKO_Kin, var.zp(auxKin_off_pos)) = speye(length(auxKin_off_pos));

% A(cons.auxKin_KO, var.zp(auxKO_off_pos)) = speye(length(auxKO_off_pos));
% A(cons.auxKin_KO, var.zp(auxKin_on_pos)) = speye(length(auxKin_on_pos));

if forceLength == 1
    A(cons.forceLength, var.zp(auxKO_on_pos)) = 1;
    A(cons.forceLength, var.zp(auxKin_off_pos)) = 1;
end
...
rhs(cons.auxKO) = 1;
rhs(cons.auxKin) = 1;
rhs(cons.auxKO_Kin) = 1;
% rhs(cons.auxKin_KO) = 1;
if forceLength == 1
...
lhs(cons.auxKO) = 1;
lhs(cons.auxKin) = 1;
lhs(cons.auxKO_Kin) = 0;
% lhs(cons.auxKin_KO) = 0;
if forceLength == 1
...
obj(var.zp(auxKin_on_pos)) = 0;
obj(var.zp(auxKin_off_pos)) = 1;
obj(var.zw) = 0;
...
while largest_mcs <= max_len_mcs && k < n_mcs && cplex.Model.rhs(cons.forceLength) <= max_len_mcs
    ...
    if n_pool ~= 0
        ...
        for j = 1:n_pool
            k = k+1;
            gMCSs{k, 1} = strcat(extractBetween(K_ind((solution(j).x(var.zp(auxKO_on_pos)))>0.9), "r_", "_auxKO"), '_KO');
%             if sum(solution(j).x(var.zp(synthetic_doseage_pos))>0.9)>0
%                 gMCSs_2{k, 1} = extractBetween(K_ind(find([zeros(numel(auxKO_Nodes),1); solution(j).x(var.zp(synthetic_doseage_pos))>0.9])),  "r_", "_input");
%                 gMCSs{k, 1} = [gMCSs{k, 1}; gMCSs_2{k, 1}];
%             end
            if sum(solution(j).x(var.zp(auxKin_off_pos))>0.9)>0
                gMCSs_2{k, 1} = strcat(extractBetween(K_ind(auxKin_off_pos(solution(j).x(var.zp(auxKin_off_pos))>0.9)), "r_", "_auxKin"), '_Kin');
%                 gMCSs_3{k, 1} = extractBetween(K_ind(find([zeros(numel(auxKin_Nodes),1); solution(j).x(var.zp(synthetic_doseage_pos))>0.9])),  "r_", "_input");
                gMCSs{k, 1} = [gMCSs{k, 1}; gMCSs_2{k, 1}];
            end
            n_cons = n_cons+1;
            sol_KO = solution(j).x(var.zp(auxKO_on_pos))>0.9;
            sol_Kin = solution(j).x(var.zp(auxKin_off_pos))>0.9;
            cplex.Model.A(n_cons, var.zp(auxKO_on_pos)) = sparse(double(sol_KO));
            cplex.Model.A(n_cons, var.zp(auxKin_off_pos)) = sparse(double(sol_Kin));
            cplex.Model.rhs(n_cons) = sum(sol_KO) + sum(sol_Kin) - 1;
